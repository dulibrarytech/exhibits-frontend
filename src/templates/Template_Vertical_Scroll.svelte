            ...
            {#each displayItems as {uuid = "", type = "", text = "", anchorId = null, is_visible = null, is_embedded = false}, index}
                ...
                <div class="exhibit-item" id={uuid}>
                    ...
                    {:else if type == ENTITY_TYPE.EXHIBIT_SUBHEADING} 
                        <Exhibit_Subheading id={anchorId} {text} styles={displayItems[index].styles || styles?.subheading || null} display={is_visible} padTop={index > 0 && displayItems[index - 1].type == ENTITY_TYPE.EXHIBIT_HEADING} on:mount-template-item={onMountTemplateItem} />
